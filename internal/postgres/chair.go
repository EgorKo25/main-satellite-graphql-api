...
func (db *DB) CreateChair(ctx context.Context, title string, input ChairCreate) (*domain.Main, error) {
	return db.create(ctx, title, domain.Chairs, func(tx pgx.Tx, main *domain.Main) error {
		tag, err := tx.Exec(ctx, `
		    INSERT INTO chairs (id, main_id, description3, type, created_at, update_at)
		    VALUES ($1, $2, $3, $4, $5, $5);
		`, main.SubID, main.ID, input.Description3, input.Type, main.CreatedAt)
		if err != nil {
			return fmt.Errorf("create chair: %w", err)
		}

		if tag.RowsAffected() != 1 {
			return fmt.Errorf("create chair: expected one changed row, got %d", tag.RowsAffected())
		}
...
func (db *DB) UpdateChair(
	...
	return db.update(ctx, mainID, title, domain.Chairs, func(tx pgx.Tx, main *domain.Main, now time.Time) error {
		...
		if err != nil {
			return fmt.Errorf("update chair: %w", err)
		}

		if tag.RowsAffected() != 1 {
			return fmt.Errorf("update chair: expected one changed row, got %d", tag.RowsAffected())
		}
...
func (db *DB) deleteChair(ctx context.Context, tx pgx.Tx, main *domain.Main, now time.Time) error {
	tag, err := tx.Exec(ctx, `
	    UPDATE chairs SET deleted_at = $3, update_at = $3
	    WHERE id = $1 AND main_id = $2 AND deleted_at IS NULL;
	`, main.SubID, main.ID, now)
	if err != nil {
		return fmt.Errorf("delete chair: %w", err)
	}

	if tag.RowsAffected() != 1 {
		return fmt.Errorf("delete chair: expected one changed row, got %d", tag.RowsAffected())
	}
