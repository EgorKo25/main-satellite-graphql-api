...
func setup(t *testing.T) *fixture {
	...
	server := httptest.NewServer(graph.NewHandler(service.New(postgres.New(pool)), slog.New(slog.DiscardHandler)))
	server.Client().Timeout = 30 * time.Second
	t.Cleanup(server.Close)

	return &fixture{pool: pool, server: server, trace: trace, database: database}
}
...
func TestListQueryCount(t *testing.T) {
	testFixture := setup(t)
	for range 25 {
		testFixture.create(t, toolBranch, "bulk", object{})
	}

	for _, limit := range []int{1, 20} {
		t.Run(fmt.Sprintf("limit_%d", limit), func(t *testing.T) {
			testFixture.trace.queries.Store(0)
			rows := testFixture.list(t, object{limitArgument: limit})
			count := testFixture.trace.queries.Load()
